...
class MechanicIndividuls extends StatefulWidget {
  final Map<String, dynamic> mechanic;

  const MechanicIndividuls({super.key, required this.mechanic});

  @override
  State<MechanicIndividuls> createState() => _MechanicIndividulsState();
}

class _MechanicIndividulsState extends State<MechanicIndividuls> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          widget.mechanic['name'] ?? 'mechanic; Profile',
      ...
      bottomNavigationBar: Padding(
        ...
        child: GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => chatscreen(workers: widget.mechanic)),
            );
          },
          child: Container(
            height: 55,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            CircleAvatar(
            radius: 80,
            backgroundImage: NetworkImage(widget.mechanic['imageUrl'] ?? ''),
            onBackgroundImageError: (_, __) => Icon(Icons.person, size: 80,
             color: Colors.black
            ),
            backgroundColor: isDark?Colors.grey[900]:Colors.teal[100],
            child: widget.mechanic['imageUrl'] == null ? const Icon(Icons.person, size: 80) : null,
                          ),
            const SizedBox(height: 20),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            _buildInfo("Name", widget.mechanic['name'], isDark),
            _buildInfo("Email", widget.mechanic['email'], isDark),
            _buildInfo("Phone", widget.mechanic['phonenumber'], isDark, isPhoneNumber: true),
            _buildInfo("City", widget.mechanic['city'], isDark),
            _buildInfo("Experience", "${widget.mechanic['experience']} years", isDark),
            _buildInfo("Category", widget.mechanic['categories']?.join(', ') ?? 'N/A', isDark),
          ],
        ),
      ),
    );
