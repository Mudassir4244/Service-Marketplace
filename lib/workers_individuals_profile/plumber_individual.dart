...
// class PlumberIndividual extends StatelessWidget {
//   final Map<String, dynamic> plumber; // Receive plumber details

//   const PlumberIndividual({super.key, required this.plumber});
...
//                     child: TextButton(
//                       onPressed: (){
//                         Navigator.push(context, MaterialPageRoute(builder: (context)=>chatscreen(workers: plumber,)));
//                       },
//                       child: Container(
//                         height: 60,
...
//               if (plumber['categoryImage'] != null) 
//                 ClipRRect(
//                   borderRadius: BorderRadius.circular(15),
//                   child: Image.asset(
//                     plumber['categoryImage'] ?? 'assets/plumber.jpeg', // Placeholder if no image
//                     height: 150,
//                     width: double.infinity,
//                     fit: BoxFit.cover,
...
//                 child: Image.network(
//                   plumber['imageUrl'] ?? 'https://via.placeholder.com/150', // Placeholder if no image
//                   height: 120,
//                   width: 120,
//                   fit: BoxFit.cover,
//                   errorBuilder: (context, error, stackTrace) {
...
//                     crossAxisAlignment: CrossAxisAlignment.start,
//                     children: [
//                       buildRichText("Name:", plumber['name'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       buildRichText("Email:", plumber['email'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       // **Tappable Phone Number**
//                       buildRichText("Phone:", plumber['phonenumber'] ?? 'N/A', isPhoneNumber: true),
//                       const SizedBox(height: 10),

//                       buildRichText("City:", plumber['city'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       buildRichText("Experience:", "${plumber['experience'] ?? 'N/A'} years"),
//                     ],
//                   ),
//                 ),
//               ),
...
//                   child: buildRichText("Category:", plumber['categories']?.join(', ') ?? 'N/A'),
//                 ))),
//                 SizedBox(
...
class PlumberIndividual extends StatefulWidget {
  final Map<String, dynamic> plumber;

  const PlumberIndividual({super.key, required this.plumber});

  @override
  State<PlumberIndividual> createState() => _PlumberIndividualState();
}

class _PlumberIndividualState extends State<PlumberIndividual> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          widget.plumber['name'] ?? 'plumber Profile',
      ...
      bottomNavigationBar: Padding(
        ...
        child: GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => chatscreen(workers: widget.plumber)),
            );
          },
          child: Container(
            height: 55,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            CircleAvatar(
            radius: 80,
            backgroundImage: NetworkImage(widget.plumber['imageUrl'] ?? ''),
            onBackgroundImageError: (_, __) => Icon(Icons.person, size: 80,
             color: Colors.black
            ),
            backgroundColor: isDark?Colors.grey[900]:Colors.teal[100],
            child: widget.plumber['imageUrl'] == null ? const Icon(Icons.person, size: 80) : null,
                          ),
            const SizedBox(height: 20),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildInfo("Name", widget.plumber['name'], isDark),
            _buildInfo("Email", widget.plumber['email'], isDark),
            _buildInfo("Phone", widget.plumber['phonenumber'], isDark, isPhoneNumber: true),
            _buildInfo("City", widget.plumber['city'], isDark),
            _buildInfo("Experience", "${widget.plumber['experience']} years", isDark),
            _buildInfo("Category", widget.plumber['categories']?.join(', ') ?? 'N/A', isDark),
          ],
        ),
      ),
    );
