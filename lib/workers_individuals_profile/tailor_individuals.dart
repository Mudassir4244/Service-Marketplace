...
// class tailorsIndividual extends StatelessWidget {
//   final Map<String, dynamic> tailors; // Receive tailors details

//   const tailorsIndividual({super.key, required this.tailors});
...
//                       onTap: (){
//                         Navigator.push(context, MaterialPageRoute(builder: (context)=>chatscreen(workers: tailors, )));
//                       },
//                       child: Container(
//                         height: 60,
...
//               if (tailors['categoryImage'] != null) 
//                 ClipRRect(
//                   borderRadius: BorderRadius.circular(15),
//                   child: Image.network(
//                     tailors['categoryImage'] ?? 'https://via.placeholder.com/300', // Placeholder if no image
//                     height: 150,
//                     width: double.infinity,
//                     fit: BoxFit.cover,
...
//                 child: Image.network(
//                   tailors['imageUrl'] ?? 'https://via.placeholder.com/150', // Placeholder if no image
//                   height: 120,
//                   width: 120,
//                   fit: BoxFit.cover,
//                   errorBuilder: (context, error, stackTrace) {
...
//                     crossAxisAlignment: CrossAxisAlignment.start,
//                     children: [
//                       buildRichText("Name:", tailors['name'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       buildRichText("Email:", tailors['email'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       // **Tappable Phone Number**
//                       buildRichText("Phone:", tailors['phonenumber'] ?? 'N/A', isPhoneNumber: true),
//                       const SizedBox(height: 10),

//                       buildRichText("City:", tailors['city'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       buildRichText("Experience:", "${tailors['experience'] ?? 'N/A'} years"),
//                     ],
//                   ),
//                 ),
//               ),
...
//                   child: buildRichText("Category:", tailors['categories']?.join(', ') ?? 'N/A'),
//                 ))),
//                 SizedBox(
...
class TailorIndividuals extends StatefulWidget {
  final Map<String, dynamic> tailor;

  const TailorIndividuals({super.key, required this.tailor});

  @override
  State<TailorIndividuals> createState() => _TailorIndividualsState();
}

class _TailorIndividualsState extends State<TailorIndividuals> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          widget.tailor['name'] ?? 'tailor; Profile',
      ...
      bottomNavigationBar: Padding(
        ...
        child: GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => chatscreen(workers: widget.tailor)),
            );
          },
          child: Container(
            height: 55,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            CircleAvatar(
            radius: 80,
            backgroundImage: NetworkImage(widget.tailor['imageUrl'] ?? ''),
            onBackgroundImageError: (_, __) => Icon(Icons.person, size: 80,
             color: Colors.black
            ),
            backgroundColor: isDark?Colors.grey[900]:Colors.teal[100],
            child: widget.tailor['imageUrl'] == null ? const Icon(Icons.person, size: 80) : null,
                          ),
            const SizedBox(height: 20),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildInfo("Name", widget.tailor['name'], isDark),
            _buildInfo("Email", widget.tailor['email'], isDark),
            _buildInfo("Phone", widget.tailor['phonenumber'], isDark, isPhoneNumber: true),
            _buildInfo("City", widget.tailor['city'], isDark),
            _buildInfo("Experience", "${widget.tailor['experience']} years", isDark),
            _buildInfo("Category", widget.tailor['categories']?.join(', ') ?? 'N/A', isDark),
          ],
        ),
      ),
    );
