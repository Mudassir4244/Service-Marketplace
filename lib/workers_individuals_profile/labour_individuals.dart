...
// class LaboursIndividual extends StatelessWidget {
//   final Map<String, dynamic> Labours; // Receive Labours details

//   const LaboursIndividual({super.key, required this.Labours});
...
//                       onTap: (){
//                         Navigator.push(context, MaterialPageRoute(builder: (context)=>chatscreen(workers: Labours,)));
//                       },
//                       child: Container(
//                         height: 60,
...
//               if (Labours['categoryImage'] != null) 
//                 ClipRRect(
//                   borderRadius: BorderRadius.circular(15),
//                   child: Image.network(
//                     Labours['categoryImage'] ?? 'https://via.placeholder.com/300', // Placeholder if no image
//                     height: 150,
//                     width: double.infinity,
//                     fit: BoxFit.cover,
...
//                 child: Image.network(
//                   Labours['imageUrl'] ?? 'https://via.placeholder.com/150', // Placeholder if no image
//                   height: 120,
//                   width: 120,
//                   fit: BoxFit.cover,
//                   errorBuilder: (context, error, stackTrace) {
...
//                     crossAxisAlignment: CrossAxisAlignment.start,
//                     children: [
//                       buildRichText("Name:", Labours['name'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       buildRichText("Email:", Labours['email'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       // **Tappable Phone Number**
//                       buildRichText("Phone:", Labours['phonenumber'] ?? 'N/A', isPhoneNumber: true),
//                       const SizedBox(height: 10),

//                       buildRichText("City:", Labours['city'] ?? 'N/A'),
//                       const SizedBox(height: 10),

//                       buildRichText("Experience:", "${Labours['experience'] ?? 'N/A'} years"),
//                     ],
//                   ),
//                 ),
//               ),
...
//                   child: buildRichText("Category:", Labours['categories']?.join(', ') ?? 'N/A'),
//                 ))),
//                 SizedBox(
...
class LabourIndividuals extends StatefulWidget {
  final Map<String, dynamic> labour;

  const LabourIndividuals({super.key, required this.labour});

  @override
  State<LabourIndividuals> createState() => _LabourIndividualsState();
}

class _LabourIndividualsState extends State<LabourIndividuals> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          widget.labour['name'] ?? 'labour Profile',
      ...
      bottomNavigationBar: Padding(
        ...
        child: GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => chatscreen(workers: widget.labour)),
            );
          },
          child: Container(
            height: 55,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            CircleAvatar(
            radius: 80,
            backgroundImage: NetworkImage(widget.labour['imageUrl'] ?? ''),
            onBackgroundImageError: (_, __) => Icon(Icons.person, size: 80,
             color: Colors.black
            ),
            backgroundColor: isDark?Colors.grey[900]:Colors.teal[100],
            child: widget.labour['imageUrl'] == null ? const Icon(Icons.person, size: 80) : null,
                          ),
            const SizedBox(height: 20),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildInfo("Name", widget.labour['name'], isDark),
            _buildInfo("Email", widget.labour['email'], isDark),
            _buildInfo("Phone", widget.labour['phonenumber'], isDark, isPhoneNumber: true),
            _buildInfo("City", widget.labour['city'], isDark),
            _buildInfo("Experience", "${widget.labour['experience']} years", isDark),
            _buildInfo("Category", widget.labour['categories']?.join(', ') ?? 'N/A', isDark),
          ],
        ),
      ),
    );
