...
class CarpentersIndividual extends StatefulWidget {
  final Map<String, dynamic> carpenter;

  const CarpentersIndividual({super.key, required this.carpenter});

  @override
  State<CarpentersIndividual> createState() => _CarpentersIndividualState();
}

class _CarpentersIndividualState extends State<CarpentersIndividual> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          widget.carpenter['name'] ?? 'Carpenter Profile',
      ...
      bottomNavigationBar: Padding(
        ...
        child: GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => chatscreen(workers: widget.carpenter)),
            );
          },
          child: Container(
            height: 55,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            CircleAvatar(
            radius: 80,
            backgroundImage: NetworkImage(widget.carpenter['imageUrl'] ?? ''),
            onBackgroundImageError: (_, __) => Icon(Icons.person, size: 80,
             color: Colors.black
            ),
            backgroundColor: isDark?Colors.grey[900]:Colors.teal[100],
            child: widget.carpenter['imageUrl'] == null ? const Icon(Icons.person, size: 80) : null,
                          ),
            const SizedBox(height: 20),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            _buildInfo("Name", widget.carpenter['name'], isDark),
            _buildInfo("Email", widget.carpenter['email'], isDark),
            _buildInfo("Phone", widget.carpenter['phonenumber'], isDark, isPhoneNumber: true),
            _buildInfo("City", widget.carpenter['city'], isDark),
            _buildInfo("Experience", "${widget.carpenter['experience']} years", isDark),
            _buildInfo("Category", widget.carpenter['categories']?.join(', ') ?? 'N/A', isDark),
          ],
        ),
      ),
    );
